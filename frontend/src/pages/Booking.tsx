...
export default function Booking() {
  // --- This holds our perfectly formatted date ---
  const [dateValue, setDateValue] = useState("");

  // --- The Smart Mask Function ---
  const handleDateInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    // 1. Strip out anything that isn't a number (blocks letters/sentences)
    let value = e.target.value.replace(/\D/g, "");

    // 2. Automatically add the slashes as they type
    if (value.length > 2) value = value.slice(0, 2) + "/" + value.slice(2);
    if (value.length > 5) value = value.slice(0, 5) + "/" + value.slice(5, 9);

    // 3. Update the input box
    setDateValue(value);
  };
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 mb-20 flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1 w-full">
            ...
            <select className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 text-gray-600 outline-none focus:ring-2 focus:ring-black appearance-none cursor-pointer">
          ...
          <div className="flex-1 w-full">
            ...
            <input 
              ...
              maxLength={10} // Stops them from typing past 10 characters
              className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 text-gray-900 outline-none focus:ring-2 focus:ring-black" 
            />
          </div>
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm mb-8">1 professional matches your search</p>

          {/* Stylist Card 1 (Jordan Lee) */}
          <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm flex flex-col md:flex-row mb-6 hover:shadow-md transition-shadow">
            ...
            <div className="md:w-72 h-72 md:h-auto">
              <img src="https://images.unsplash.com/photo-1615163012209-6616016e344e?auto=format&fit=crop&q=80&w=800" alt="Jordan Lee" className="w-full h-full object-cover" />
            </div>
            ...
            <div className="p-6 md:p-8 flex-1 flex flex-col">
              ...
              <div className="flex justify-between items-start mb-2">
                <div>
                  <h3 className="text-2xl font-medium">Jordan Lee</h3>
                  <p className="text-gray-500">Crown & Glory</p>
                  <p className="text-gray-400 text-sm mt-1">Braids & Protective Styles</p>
                </div>
                <div className="flex items-center text-yellow-500 bg-yellow-50 px-2 py-1 rounded-md">
                  ...
                  <span className="text-gray-800 font-medium text-sm">4.8 <span className="text-gray-400 font-normal">(189)</span></span>
                </div>
              </div>
              ...
              <div className="flex items-center text-gray-500 text-sm mb-6">
                ...
                <span className="mr-6">2.5 mi</span>
                <span className="font-medium text-gray-700">Starting at $120+</span>
              </div>

              <div className="mb-6">
                <p className="text-sm font-medium text-gray-700 mb-3">Available Times</p>
                <div className="flex flex-wrap gap-2">
                  <button className="border border-gray-200 text-gray-600 rounded-lg px-4 py-2 text-sm hover:border-black hover:text-black transition-colors">Tomorrow 9:00 AM</button>
                  <button className="border border-gray-200 text-gray-600 rounded-lg px-4 py-2 text-sm hover:border-black hover:text-black transition-colors">Tomorrow 1:00 PM</button>
                </div>
              </div>
